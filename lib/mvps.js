...
export default function useMvps(){
    ...
    const getMostReposts = async () => {
      ...
      const getMostLikes = async () => {
        ...
        const mostLikes = [...userObjects.values()]?.filter((user) => userLikeCount.get(user.id) === maxLikes)
          .map((user) => ({
            ...user,
            likeCount: maxLikes,
          }));
      ...
      const getMostViews = async () => {
        ...
        const mostViews = [...userObjects.values()]?.filter((user) => userViewCount.get(user.id) === maxViews)
          .map((user) => ({
            ...user,
      ...
      const getMostFollows = async () => {
        ...
        const mostFollows = [...userObjects.values()]?.filter((user) => followCount.get(user.id) === maxFollows)
          .map((user) => ({
            ...user,
      ...
      const getMostReferrals = async () => {
        ...
        const mostReferrals = [...userObjects.values()]?.filter((user) => referralCount.get(user.username) === maxReferrals)
          .map((user) => ({
            ...user,
