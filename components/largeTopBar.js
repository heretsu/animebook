...
export const TopBarObjects = () => {
  ...
  const { followingPosts, setFollowingPosts, originalPostValues, postValues, setPostValues, userNumId, setSearchFilter, setTagsFilter, originalExplorePosts,
    setExplorePosts } = useContext(UserContext);
  const searchForItem = (e) => {
    if (router.pathname === "/explore"){
      const foundItems = originalExplorePosts.filter(post => 
        post[0].content.toLowerCase().includes(e.target.value) || post[0].content.toUpperCase().includes(e.target.value) || post[0].users.username.toLowerCase().includes(e.target.value) || post[0].users.username.toUpperCase().includes(e.target.value)
      );
      setTagsFilter(false)
      ...
      const foundItems = originalPostValues.filter(post => 
        post.content.toLowerCase().includes(e.target.value) || post.content.toUpperCase().includes(e.target.value) || post.users.username.toLowerCase().includes(e.target.value) || post.users.username.toUpperCase().includes(e.target.value)
      );
      setTagsFilter(false)
      setSearchFilter(true)
...
return {followingPosts, changePostsDisplayed, getAllPosts, searchForItem}
}
    ...
    >
      ...
      {middleTab && relationship && !mobileSearchToggle && (
        <div className="flex flex-row font-semibold">
          <div onClick={getAllPosts} className=
          {followingPosts ? "text-sm md:text-base py-2 px-4 md:py-3 md:px-9 cursor-pointer rounded-l-xl bg-gray-100 flex items-center border border-gray-200 text-gray-500" : "text-sm md:text-base py-2 px-4 md:py-3 md:px-9 cursor-pointer flex flex-row bg-pastelGreen text-white items-center rounded-l-xl space-x-1"}>
            <span>For</span>
            <span>You</span>
          </div>
          <div onClick={changePostsDisplayed} className={followingPosts ? "text-sm md:text-base py-2 px-4 md:py-3 md:px-9 cursor-pointer flex flex-row bg-pastelGreen text-white items-center rounded-r-xl space-x-1" : "text-sm md:text-base py-2 px-4 md:py-3 md:px-9 cursor-pointer rounded-r-xl bg-gray-100 flex items-center border border-gray-200 text-gray-500"}>
            Following
          </div>
        </div>
      )}
      {middleTab && relationship ? (
        <>
          ...
          <span onClick={()=>{setMobileSearchToggle(true)}} className={mobileSearchToggle ? "hidden" : "flex sm:hidden flex-row justify-end items-center"}>
            <svg
          ...
          <span className={mobileSearchToggle ? "w-full" : "hidden sm:flex w-full"}>
