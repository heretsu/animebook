...
function TheaterMode({
  ...
  return (
    <div
      className={`${
        !expandStream ? "pb-20 flex w-full overflow-y-scroll" : "fixed"
      } sm:pb-0`}
      style={{width: expandStream ? "95vw" : "100%", ...(window.innerWidth <= 1024 && { width: "100%" })}}
            ...
            <div className="relative w-full h-full">
              {isScreenSharing ? (
                <div
                  ...
                  style={expandStream ? { height: "90vh" } : { height: "80vh" }}
                >
                  <VideoTrack
                ...
                cameraTrack && (
                  <div
                    className="w-full h-full bg-black"
                    style={
                      expandStream ? { height: "90vh" } : { height: "80vh" }
                    }
                  >
                    <VideoTrack
...
export default function StreamViewer({
  ...
  return (
    <div
      className={`${
        expandStream ? "w-full" : "relative h-screen overflow-y-scroll"
      }`}
    >
      <div className="p-2 flex flex-row space-x-1 justify-center items-center absolute top-4 left-1 z-50">
        ...
        <span className="flex flex-row space-x-1 bg-black bg-opacity-50 backdrop-blur-sm text-white text-sm font-semibold px-2 py-1 rounded z-10">
          <span>{viewerCount}</span> <span>{parseInt(viewerCount) == 1 ? "viewer" : "viewers"}</span>
        </span>
      </div>
