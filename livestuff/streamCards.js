...
export default function StreamCards({setOnOpenCustomizeStream, setOnOpenScheduler, darkMode}) {
  ...
  return (
    <div className="relative h-full w-full flex flex-row items-center">
      ...
      >
        <div onClick={()=>{setOnOpenCustomizeStream(true)}} className={`cursor-pointer group relative w-[30%] flex-shrink-0 ${darkMode ? 'bg-[#1e1f24]' : 'bg-white'} backdrop-blur-sm rounded-xl rounded-xl p-6 transition-all duration-300 hover:scale-105`}>
