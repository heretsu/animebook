    ...
    <div className="w-full flex justify-between items-center space-x-1">
      ...
      <span className="flex flex-row items-center space-x-3">
        ...
        <button
          ...
          className={`text-xs font-semibold text-white flex flex-row items-center px-2 py-2 rounded transition`}
          style={{
...
function HostLayout({ onEndStream, stream, userData, darkMode, viewerCount }) {
  ...
  return (
    ...
    >
      ...
      <div className="bg-black flex flex-col w-full" style={{ height: "85%" }}>
      ...
      <div
        className={`p-2 flex flex-col ${
          darkMode ? "bg-[#17181C] text-white" : "bg-white text-black"
        }`}
      >
        <CustomControls onEndStream={onEndStream} />
        <span onClick={() => {
              handleCopy(
                `https://animebook.io/watch/stream-${userData.useruuid}`
              );
            }} className="flex flex-row items-center pt-1 space-x-1 text-xs">
          <span
            ...
            } cursor-pointer shadow-xl relative flex flex-row justify-between items-center"
                  } space-x-0.5 rounded-xl`}
          >
            {`https://animebook.io/watch/stream-${userData.useruuid}`}
            {copied && (
              <span className="absolute -top-7 left-0 right-0 mx-auto w-fit bg-blue-400 text-white px-2 py-1 rounded-md whitespace-nowrap text-center">
                {"Copied!"}
              </span>
            )}
          </span>
...
export default function LiveStream({
  ...
  return (
    <div className={`relative flex w-full h-full overflow-y-scroll`}>
      {isConnected && (
        <div className="ml-5 mt-2 absolute top-4 left-1 z-10 bg-black bg-opacity-50 backdrop-blur-sm text-white text-sm font-semibold px-2 py-1 rounded">
